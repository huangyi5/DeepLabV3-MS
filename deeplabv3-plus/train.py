...
if __name__ == "__main__":
    ...
    #                   自己需要的分类个数+1，如2+1（背景）-----3+1：桥身、柱、梁盖、背景
    #-----------------------------------------------------#
    num_classes     = 4
    #---------------------------------#
    #   所使用的的主干网络：
    ...
    #----------------------------------------------------------------------------------------------------------------------------#
    model_path      = "model_data/deeplab_mobilenetv2.pth"  # xception_pytorch_imagenet.pth  /deeplab_mobilenetv2.pth
    # model_path = " "
    #---------------------------------------------------------#
    #   downsample_factor   下采样的倍数8（3次下采样）、16（四次下采样）
    #                       8下采样的倍数较小、理论上效果更好。
    #                       但也要求更大的显存------8需要用服务器跑,效果下降
    #---------------------------------------------------------#
    ...
    #   Freeze_batch_size   模型冻结训练的batch_size--->显存不足时可以设置得小一些
    #                       (当Freeze_Train=False时失效)----8，4
    #------------------------------------------------------------------#
    Init_Epoch          = 0
    Freeze_Epoch        = 50
    Freeze_batch_size   = 4
    ...
    #   UnFreeze_Epoch          模型总共训练的epoch，默认训练50个epoch=100-50
    #   Unfreeze_batch_size     模型在解冻后的batch_size = 8/4
    #------------------------------------------------------------------#
    UnFreeze_Epoch      = 100
    Unfreeze_batch_size = 4
    #------------------------------------------------------------------#
    #   Freeze_Train    是否进行冻结训练
    #                   默认先冻结主干训练后解冻训练。--即是否要分两个阶段训练
    #------------------------------------------------------------------#
    Freeze_Train        = True  # False
    ...
    #                   adam会导致weight_decay错误，使用adam时建议设置为0,使用sgd时建议设置为1e-4.
    #------------------------------------------------------------------#
    optimizer_type      = "adam"
    momentum            = 0.9
    weight_decay        = 0
